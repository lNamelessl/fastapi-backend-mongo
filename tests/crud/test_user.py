...
def test_authenticate_user(db: Database) -> None:
    ...
    user_in = UserCreate(email=email, password=password)
    user = crud.create_user(db=db, user_create=user_in)
    authenticated_user = crud.authenticate(db=db, email=email, password=password)
...
def test_not_authenticate_user(db: Database) -> None:
    email = random_email()
    password = random_lower_string()
    user = crud.authenticate(db=db, email=email, password=password)
    assert user is None
...
def test_get_user(db: Database) -> None:
    ...
    user = crud.create_user(db=db, user_create=user_in)
    user_2 = crud.get_user(db=db, user_id=user.id)
    assert user_2
...
def test_update_user(db: Database) -> None:
    ...
    if user.id is not None:
        crud.update_user(db=db, db_user=user, user_in=user_in_update)
    user_2 = crud.get_user(db=db, user_id=user.id)
    assert user_2
    assert user.email == user_2.email
...
def test_authenticate_user_with_bcrypt_upgrades_to_argon2(db: Database) -> None:
    ...
    user = crud.add_user(db=db, user=User(email=email, hashed_password=bcrypt_hash))
