...
def test_get_existing_user_as_superuser(
    ...
    assert 200 <= r.status_code < 300
    api_user = r.json()
    existing_user = crud.get_user_by_email(db=db, email=username)
...
def test_retrieve_users(
    ...
    crud.create_user(db=db, user_create=user_in2)
...
def test_update_user_me(
    ...
    user_db = crud.get_user_by_email(db=db, email=email)
    assert user_db
...
def test_update_password_me(
    ...
    user_db = crud.get_user_by_email(db=db, email=settings.FIRST_SUPERUSER)
    assert user_db
...
def test_register_user(client: TestClient, db: Database) -> None:
    ...
    user_db = crud.get_user_by_email(db=db, email=username)
    assert user_db
...
def test_update_user_email_exists(
    ...
    password2 = random_lower_string()
    user_in2 = UserCreate(email=username2, password=password2)
    user2 = crud.create_user(db=db, user_create=user_in2)
...
def test_delete_user_current_super_user_error(
    ...
    super_user = crud.get_user_by_email(db=db, email=settings.FIRST_SUPERUSER)
    assert super_user
