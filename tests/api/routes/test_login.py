...
def test_reset_password(client: TestClient, db: Database) -> None:
    ...
    create_user = crud.create_user(db=db, user_create=user_create)
    ...
    user = crud.get_user(db=db, user_id=create_user.id)
    assert user
...
def test_login_with_bcrypt_password_upgrades_to_argon2(
    ...
    user = crud.add_user(
        db=db, user=User(email=email, hashed_password=bcrypt_hash, is_active=True)
    )
...
def test_login_with_argon2_password_keeps_hash(
    ...
    user = crud.add_user(
        db=db, user=User(email=email, hashed_password=argon2_hash, is_active=True)
    )
